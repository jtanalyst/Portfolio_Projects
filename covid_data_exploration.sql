...
SELECT Location, Population, MAX(total_cases) as total_cases_july,  MAX((total_cases/ NULLIF(population,0)))*100 as percent_pop_infected
FROM Portfolio_project..covid_deaths
GROUP BY Location, Population
ORDER BY percent_pop_infected desc;

-- Highest Mortality Rate by Country (as of July 3rd, 2022)
-- Calculated by total deaths over population for each country
